jQuery(function ($) {
    ...
    $(document).ready(function () {
        ...
        let termsObj = {
            ...
            termElement3: {
                id: 'termElement3',
                termNmae: "В каком городе Вы проживаете?",
                termOptions: [
                    'Абакан',
                    'Альметьевск',
                    'Ангарск',
            ...
            termElement4: {
                id: 'termElement4',
                termNmae: "Когда Вы в последний раз покупали товары для спорта, туризма и активного отдыха (снаряжение и/или инвентарь), а также одежду и/или обувь для спорта или спортивного стиля?",
                termOptions: [
                    'В течение 3 месяцев',
                    'В течение 6 месяцев',
            ...
            termElement5: {
                id: 'termElement5',
                termNmae: "Принимаете ли Вы самостоятельно решение, где (в каком магазине) покупать товары для спорта, туризма и активного отдыха, одежду и обувь для спорта или спортивного стиля?",
                termOptions: [
                    'Да',
                    'Нет',
                ]
            ...
            termElement6: {
                id: 'termElement6',
                termNmae: "Укажите Ваш уровень дохода",
                termOptions: [
                    'Низкий (не хватает денег на одежду, еду, оплату жилья)',
                    'Средний – (хватает денег на еду и одежду, но не можем покупать дорогие вещи)',
                    'Высокий (могу себе позволить покупку автомобиля, путешествия)',
                    'Отказ от ответа',
                ]
        ...
        let answerGrouphelp =
            `<div class="tooltip">
                <p class="tooltip-header">Добавление группы ответов</p>
                <p>
                    В опрос можно быстро добавить большое количество вариантов ответов, скопировав и вставив сразу целый список. Каждый вариант ответа должен отображаться в отдельной строке.
                </p>
            </div>`
        ...
        function addQuestion(type, appendInde, id) {
            ...
            let termsSelectedListHTML = '';
            let counter = 1;
            let elementNameCounter = 0;
            for (let element of Object.values(termsObj)) {
                elementNameCounter++
                termsSelectedListHTML += `<span class="wrapper ${element.id}">
                <span>Q-${elementNameCounter} :</span> <span class="selected-radio-list"></span>; 
                </span>`;
                ...
                for (let submenu of element.termOptions) {
                    termsSubmenuTemplate +=
                        `<li class="menu-item sub-menu-item">
                            <div>
                                <input type="checkbox" id="term_${id}_${counter}" name="term_${id}_${counter}" class="term-el" value="${submenu}" data-key="${element.id}" />
                                <label for="term_${id}_${counter}">${submenu}</label>
                            </div>
                        </li>`
                    counter++;
                }
                termsListTemplate += `<li class="menu-item main-menu-item">
                <div class="main-term"><span>${elementNameCounter}. </span>${element.termNmae}</div>
                <ul class="sub-menu">
                <li class="menu-item sub-menu-item check-all-terms">
                    <div>
            ...
            let terms =
                ...
                <div class="terms-box">
                    <ul class="terms-main-menu menu" data-index="${termIndex}">
                       ${termsListTemplate}
                    </ul>
                </div>
            </div>`
            ...
            let topEL =
                ...
                <div class="display-checked-terms">${termsSelectedListHTML}</div>
                ${terms}
                ${nameHtml}`;
            let el;
            let required_Set =
        ...
        /** Check all terms */
        $('.constr-wrap').on('click', '.check-all-terms', function () {
            $(this).toggleClass('checked');
            const index = $(this).parents('.terms-main-menu').attr('data-index');
            console.log('term-index: ', index)
            if ($(this).hasClass('checked')) {
                $(this).parents('.sub-menu').find('.term-el').prop('checked', true);
                let allCheckedTerms = $(this).parents('.sub-menu').find('.term-el:checked');
                $(allCheckedTerms).trigger("change");
        ...
        $('.constr-wrap').on('change', '.term-el', function () {
            const termKey = $(this).attr('data-key');
            const questionContent = $(this).closest('.question-content');
            const allCheckedTerms = $(this).parents('.sub-menu').find('.term-el:checked');
            let valuesList = []
            allCheckedTerms.each(function () {
                valuesList.push($(this).val());
            })
            $(questionContent).find(`.${termKey} .selected-radio-list`).text(valuesList.join(', '));
            $(questionContent).find(`.${termKey}`).show();
            if (allCheckedTerms.length == 0) {
                $(questionContent).find(`.${termKey}`).hide();
            }
        });
        ...
        $('.constr-wrap').on('mouseover', '.help-icon', function () {
            $(this).parents('.answer-group-btn').find('.tooltip').fadeIn();
        });
        $('.constr-wrap').on('mouseout', '.help-icon', function () {
            $(this).parents('.answer-group-btn').find('.tooltip').fadeOut();
        });

        let currentQuestionBlock = null;
        $('.constr-wrap').on('click', '.answer-group-btn', function () {
            currentQuestionBlock = $(this).closest('.question-wrap');
            $('.answers-group_popup').fadeIn(300);
        });

        // Answer group popup settings

        $('.save-answers-group').on('click', function () {
            console.log($('.load-wrapper'))
            let thisQuestion = currentQuestionBlock;
            let text = $(this).parents('.popup-body').find('#add-answerd-array').val();
            const groupAnwers = text.split(/\n/);
            if (text) {
                if (groupAnwers.length > 200) {
                    $('.limit-txt').css('color', '#FF7262');
                } else {
                    $('.limit-txt').css('color', '#000');
                    ...
                    $.each(groupAnwers, function (i, line) {
                        if (line && line.length) {
                            addSingleOption(questionId, pointId, line, itemsList);
                        }
                    });
                    $('.answers-group_popup').fadeOut(300);
                    $(this).parents('.popup-body').find('#add-answerd-array').val('');
                }
            } else {
                $(this).parents('.popup-body').find('#add-answerd-array').css('border-color', '#FF7262');
            }
        })

        $('.close-answers-group').on('click', function () {
            $(this).parents('.popup-body').find('#add-answerd-array').val('');
            $('.answers-group_popup').fadeOut(300);
        });
        ...
        $('#add-answerd-array').on('focus', function () {
            $(this).css('border-color', '#7FA1F8');
        })
        $('#add-answerd-array').on('focusout', function () {
